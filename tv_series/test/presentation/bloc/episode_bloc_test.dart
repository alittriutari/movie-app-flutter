...
void main() {
  ...
  blocTest<EpisodeBloc, EpisodeState>('emits [Loading, Loaded] when  data is gotten successfully.',
      ...
      act: (bloc) => bloc..add(const GetEpisodeEvent(id: tId, seasonNumber: tSeason)),
  ...
  blocTest<EpisodeBloc, EpisodeState>('emits [Loading, Failure] when  data is unsuccessful.',
      build: () {
        when(mockGetTvEpisode.execute(tId, tSeason)).thenAnswer((_) async => const Left(ServerFailure('')));
        return episodeBloc;
      },
      act: (bloc) => bloc..add(const GetEpisodeEvent(id: tId, seasonNumber: tSeason)),
      wait: const Duration(milliseconds: 100),
      expect: () => [EpisodeLoading(), const EpisodeFailure(failure: ServerFailure(''))],
      verify: (bloc) {
        verify(mockGetTvEpisode.execute(tId, tSeason));
      });
}
