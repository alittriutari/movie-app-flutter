...
void main() {
  ...
  blocTest<TopRatedMovieBloc, TopRatedMovieState>('emits [Loading, Failure] when  data is unsuccessful.',
      build: () {
        when(mockGetTopRatedMovies.execute()).thenAnswer((_) async => const Left(ServerFailure('')));
        return topRatedMovieBloc;
      },
      act: (bloc) => bloc..add(GetTopRatedMovieListEvent()),
      wait: const Duration(milliseconds: 100),
      expect: () => [TopRatedMovieLoading(), const TopRatedMovieFailure(failure: ServerFailure(''))],
      verify: (bloc) {
        verify(mockGetTopRatedMovies.execute());
      });
}
